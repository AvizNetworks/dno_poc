...
// Helper to parse SNI from TLS ClientHello
static int parse_sni(const unsigned char *data, int data_len, char *sni, size_t sni_len) {
    int pos = 0;
    // TLS record header: 5 bytes
    if (data_len < 5 || data[0] != 0x16) return -1; // 0x16 = Handshake
    pos += 5;
    // Handshake header: 1 byte type, 3 bytes length
    if (data_len < pos + 4 || data[pos] != 0x01) return -1; // 0x01 = ClientHello
    pos += 4;
    // Skip: version(2), random(32), session id len(1), session id(?), cipher suites len(2), cipher suites(?), compression len(1), compression(?)
    if (data_len < pos + 2) return -1;
    pos += 2 + 32; // version + random
    if (data_len < pos + 1) return -1;
    int session_id_len = data[pos]; pos += 1;
    pos += session_id_len;
    if (data_len < pos + 2) return -1;
    int cipher_suites_len = (data[pos] << 8) | data[pos+1]; pos += 2;
    pos += cipher_suites_len;
    if (data_len < pos + 1) return -1;
    int compression_len = data[pos]; pos += 1;
    pos += compression_len;
    if (data_len < pos + 2) return -1;
    int extensions_len = (data[pos] << 8) | data[pos+1]; pos += 2;
    ...
    while (pos + 4 <= extensions_end && pos + 4 <= data_len) {
        ...
        if (ext_type == 0x00) { // SNI
            ...
            int sni_list_len = (data[pos] << 8) | data[pos+1]; pos += 2;
            if (pos + 1 > data_len) return -1;
            int sni_type = data[pos]; pos += 1;
            if (sni_type != 0x00) return -1;
            if (pos + 2 > data_len) return -1;
            int sni_len_val = (data[pos] << 8) | data[pos+1]; pos += 2;
            if (pos + sni_len_val > data_len) return -1;
            if (sni_len_val >= sni_len) return -1;
            memcpy(sni, &data[pos], sni_len_val);
            sni[sni_len_val] = '\0';
            return 0;
        }
        pos += ext_len;
...
void handle_client(int client_sock, pcap_dumper_t* dumper) {
 ...
 uint32_t orig_dst_ip_uint32=0; 
    ...
    if (getsockopt(client_sock, SOL_IP, SO_ORIGINAL_DST, &orig_dst, &orig_dst_len) != 0) {
    ...
    int orig_port = ntohs(orig_dst.sin_port);
    debug_print("Original destination: %s:%d\n", orig_ip, orig_port);

    // Step 2: Read ClientHello and parse SNI
    unsigned char hello_buf[2048];
    int hello_len = recv(client_sock, hello_buf, sizeof(hello_buf), MSG_PEEK);
    if (hello_len <= 0) {
    ...
    char sni[256] = {0};
    if (parse_sni(hello_buf, hello_len, sni, sizeof(sni)) != 0) {
        debug_print("SNI not found in ClientHello\n");
        // Optionally, fallback to orig_ip
        strncpy(sni, orig_ip, sizeof(sni)-1);
    } else {
        debug_print("Parsed SNI: %s\n", sni);
    }

    // Step 3: Generate per-domain cert and key
    X509* leaf_cert = NULL;
    EVP_PKEY* leaf_key = NULL;
    if (generate_cert(sni, &leaf_cert, &leaf_key) != 0) {
        fprintf(stderr, "Failed to generate cert for %s\n", sni);
        close(client_sock);
        return;
    }

    // Step 4: Create per-connection SSL_CTX in server mode
    ...
    // Step 5: Accept TLS handshake from client
    ...
    // Step 6: Connect to real server (upstream)
    SSL* server_ssl;
    int server_sock = connect_to_server(sni, orig_port, &server_ssl);
    if (server_sock < 0) {
        SSL_free(client_ssl);
        SSL_CTX_free(client_ctx);
        EVP_PKEY_free(leaf_key);
