...
#
#   ./make_packages.sh [output-dir]        # default: ~/work/fable/hbn-packages-<YYYYMMDD>
#
# Produces:
...
DATE=$(date +%Y%m%d)
OUT="${1:-${HOME}/work/fable/hbn-packages-${DATE}}"

echo "== staging into ${OUT}"
rm -rf "${OUT}"
mkdir -p "${OUT}/hbn-standalone/scripts" "${OUT}/hbn-dpf/dpf"

# ── Standalone packet ─────────────────────────────────────────────────────────
cp -r "${REPO}/mellanox" "${REPO}/doca_hbn_v3.3.0" "${REPO}/docs" "${OUT}/hbn-standalone/"
for f in bringup_hbn_bf3.sh status_hbn.sh topology_hbn.sh access_hbn.sh \
  ...
  cp "${REPO}/scripts/${f}" "${OUT}/hbn-standalone/scripts/"
done

# Generate the customer README: strip lab-internal blocks
python3 - "${REPO}/README.md" "${OUT}/hbn-standalone/README.md" <<'PY'
import sys, re
...
for f in README.md QUICKSTART.md CHEATSHEET.md MULTI-DPU-DESIGN.md \
  ...
  cp "${REPO}/dpf/${f}" "${OUT}/hbn-dpf/dpf/"
done
cp -r "${REPO}/dpf/manifests" "${REPO}/dpf/scripts" "${OUT}/hbn-dpf/dpf/"
rm -f "${OUT}/hbn-dpf/dpf/scripts/"*.bak \
      "${OUT}/hbn-dpf/dpf/scripts/nvidia_debug_commands.sh" 2>/dev/null || true
...
# ── Tarballs + checksums ──────────────────────────────────────────────────────
cd "${OUT}"
tar czf "hbn-standalone-${DATE}.tar.gz" hbn-standalone
tar czf "hbn-dpf-${DATE}.tar.gz" hbn-dpf
