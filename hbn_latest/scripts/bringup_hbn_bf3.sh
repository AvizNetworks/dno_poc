...
info "Step 1/14 — Verifying eswitch switchdev mode"
...
info "Step 3/14 — Checking apparmor-utils"
...
info "Step 4/14 — Creating hostPath directories"
mkdir -p \
...
info "Step 5/14 — Deploying reference config files from ${MLX_REF_DIR}"
...
SF_CONF_BAD=false
if [[ -n "$P0_MAC" ]] && grep -qi "$P0_MAC" "${MLX_SF_CONF}" 2>/dev/null; then
  warn "mlnx-sf.conf assigns physical p0 MAC ${P0_MAC} to an SF — replacing from reference"
  SF_CONF_BAD=true
elif [[ -n "$P1_MAC" ]] && grep -qi "$P1_MAC" "${MLX_SF_CONF}" 2>/dev/null; then
  warn "mlnx-sf.conf assigns physical p1 MAC ${P1_MAC} to an SF — replacing from reference"
  SF_CONF_BAD=true
fi

if [[ "$SF_CONF_BAD" == "true" ]] || [[ ! -f "${MLX_SF_CONF}" ]]; then
  cp "${MLX_REF_DIR}/mlnx-sf.conf" "${MLX_SF_CONF}"
  ok "mlnx-sf.conf installed from reference"
else
  ok "mlnx-sf.conf MAC check passed"
fi
...
# ─── Step 6: Hugepages ───────────────────────────────────────────────────────
info "Step 6/14 — Hugepage allocation (OVS-DPDK requires 1600×2MB)"
...
# ─── Step 7: Provision SFs ───────────────────────────────────────────────────
info "Step 7/14 — Checking SF provisioning (sfnum 2, 3, 1514, 1515)"

SFS_PROVISIONED=false
SF_NEEDS_REPROVISION=false

if devlink port show 2>/dev/null | grep -q "sfnum 2"; then
  # SFs exist — check that function netdevs were created (missing = MAC conflict)
  MISSING_NETDEVS=()
  ...
  if [[ ${#MISSING_NETDEVS[@]} -gt 0 ]]; then
    warn "SFs provisioned but function netdevs missing: ${MISSING_NETDEVS[*]}"
    warn "MAC conflict with physical port — deleting SFs for reprovisioning"
    SF_NEEDS_REPROVISION=true
  else
    ok "SFs already provisioned with correct function netdevs"
    SFS_PROVISIONED=true
  fi
fi

if [[ "$SF_NEEDS_REPROVISION" == "true" ]]; then
  info "Deleting existing HBN SFs..."
...
if [[ "$SFS_PROVISIONED" == "false" ]]; then
  ...
  for SFNUM in 2 3 1514 1515; do
    OPSTATE=$(devlink port show 2>/dev/null | grep "sfnum ${SFNUM}" | grep -o "opstate [a-z]*" | awk '{print $2}' || echo "")
    if [[ "$OPSTATE" != "attached" ]]; then
      warn "sfnum ${SFNUM} opstate: ${OPSTATE:-unknown} (expected: attached)"
      SF_OK=false
    fi
  done
  for NETDEV in p0_if_r p1_if_r pf0hpf_if_r pf1hpf_if_r; do
    if [[ ! -d "/sys/class/net/${NETDEV}" ]]; then
      warn "Function netdev ${NETDEV} not present after SF provisioning"
      SF_OK=false
    fi
  done

  [[ "$SF_OK" == "true" ]] \
    && ok "SFs provisioned: all 4 attached with function netdevs" \
    || warn "SF state may still be settling — check 'mlnx-sf -a show' if pod fails to start"
fi

# ─── Step 8: OVS health check ────────────────────────────────────────────────
info "Step 8/14 — OVS bridge health check"

OVS_RESTARTED=false

if ovs-vsctl show 2>/dev/null | grep -q "Invalid argument"; then
...
if [[ "$OVS_RESTARTED" == "true" ]] || [[ "$SFS_PROVISIONED" == "false" ]]; then
  info "Restarting sfc.service to wire up SFs and hugepages..."
  systemctl restart sfc.service
  sleep 20
fi

# ─── Step 9: Validate OVS ports ──────────────────────────────────────────────
info "Step 9/14 — Validating OVS ports on br-hbn"
...
if [[ -n "$OVS_MISSING" ]]; then
  warn "Missing OVS ports: ${OVS_MISSING} — restarting sfc.service"
  systemctl restart sfc.service
  sleep 20
  OVS_MISSING=$(check_ovs_ports)
  if [[ -n "$OVS_MISSING" ]]; then
    warn "Still missing OVS ports: ${OVS_MISSING} — check: journalctl -u sfc -n 50"
  else
    ok "All OVS ports present after sfc.service restart"
  fi
else
  ok "All 8 OVS ports present on br-hbn"
fi

# ─── Step 10: Pull container image ───────────────────────────────────────────
info "Step 10/14 — Checking doca_hbn container image"
...
# ─── Step 11: Wait for doca-hbn pod ──────────────────────────────────────────
info "Step 11/14 — Waiting for doca-hbn container to be Running (timeout: ${WAIT_TIMEOUT}s)"
ELAPSED=0
while true; do
...
# ─── Step 12: Bring up interfaces inside container ───────────────────────────
info "Step 12/14 — Bringing up HBN interfaces inside container"
...
# ─── Step 13: Enable BGP ─────────────────────────────────────────────────────
info "Step 13/14 — BGP configuration"
...
# ─── Step 14: REST API — password + listening address ────────────────────────
info "Step 14/14 — REST API setup"
...
printf "  %-25s %s\n" "SFs provisioned:" "$SF_COUNT (expect 5)"
