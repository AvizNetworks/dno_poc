...
# ─── Log file ────────────────────────────────────────────────────────────────
LOG_FILE="hbn_validate_$(date +%Y%m%d_%H%M%S).log"
...
if [[ "$SETUP" == "true" ]]; then
  ...
  info "Configuring BF3 ${BF3_P0_IFACE} = ${BF3_P0_CIDR}"
  bf3_cont_cmd "ip link set ${BF3_P0_IFACE} up" > /dev/null 2>&1 || true
  bf3_cont_cmd "ip addr add ${BF3_P0_CIDR} dev ${BF3_P0_IFACE}" > /dev/null 2>&1 || true
  BF3_P0_CHECK=$(bf3_cont_cmd "ip addr show ${BF3_P0_IFACE}" | grep "inet ${BF3_P0_IP}" || true)
  [[ -n "$BF3_P0_CHECK" ]] && ok "BF3: ${BF3_P0_IFACE} = ${BF3_P0_CIDR}" || fail "BF3: ${BF3_P0_IFACE} not configured"

  info "Configuring BF3 ${BF3_PF0_IFACE} = ${BF3_PF0_CIDR}"
  bf3_cont_cmd "ip link set ${BF3_PF0_IFACE} up" > /dev/null 2>&1 || true
  bf3_cont_cmd "ip addr add ${BF3_PF0_CIDR} dev ${BF3_PF0_IFACE}" > /dev/null 2>&1 || true
  BF3_PF0_CHECK=$(bf3_cont_cmd "ip addr show ${BF3_PF0_IFACE}" | grep "inet ${BF3_PF0_IP}" || true)
  [[ -n "$BF3_PF0_CHECK" ]] && ok "BF3: ${BF3_PF0_IFACE} = ${BF3_PF0_CIDR}" || fail "BF3: ${BF3_PF0_IFACE} not configured"
...
# 2. BF3 → Host
info "Ping: BF3 pf0hpf_if (${BF3_PF0_IP}) → Host ${HOST_IFACE} ($(echo $HOST_IFACE_IP | cut -d/ -f1))"
...
info "Ping: Host ${HOST_IFACE} → BF3 pf0hpf_if (${BF3_PF0_IP})"
...
if [[ -n "$RT" ]]; then
  ...
  info "Routing table empty — no IPs configured on container interfaces"
fi
