...
if ovs-vsctl show 2>/dev/null | grep -q "br-hbn"; then
  ...
  ERRORS=$(ovs-vsctl show 2>/dev/null | grep -c "No such device\|error\|could not" || true)
  if [[ "$ERRORS" -gt 0 ]]; then
    fail "OVS has $ERRORS port error(s) — run: sudo systemctl restart openvswitch-switch"
    ovs-vsctl show 2>/dev/null | grep -E "No such device|error|could not" | while read -r line; do
      echo "       $line"
    done
  else
