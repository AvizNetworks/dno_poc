...
# Configure ns_src
ip netns exec ns_src ip addr add 10.0.1.1/24 dev veth_src_ns
ip netns exec ns_src ip link set veth_src_ns up
ip netns exec ns_src ip link set lo up

# Configure ns_dst
ip netns exec ns_dst ip addr add 10.0.2.1/24 dev veth_dst_ns
ip netns exec ns_dst ip link set veth_dst_ns up
ip netns exec ns_dst ip link set lo up

# Configure host-side endpoints
ip addr add 10.0.1.2/24 dev veth_src_host
ip link set veth_src_host up
ip addr add 10.0.2.2/24 dev veth_dst_host
ip link set veth_dst_host up

# Disable reverse path filtering (required for forwarding to work)
...
echo "  ns_src:  veth_src_ns  (10.0.1.1/24)"
echo "  host:    veth_src_host (10.0.1.2/24)"
echo "  host:    veth_dst_host (10.0.2.2/24)"
echo "  ns_dst:  veth_dst_ns  (10.0.2.1/24)"
