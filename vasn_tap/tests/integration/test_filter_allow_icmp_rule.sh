#!/bin/bash
#
# vasn_tap integration test - Filter: default drop, one rule allow protocol=icmp
# Expect: RX>0, TX>0, Dropped=0 (ICMP allowed by rule).
# Usage: test_filter_allow_icmp_rule.sh [afpacket|ebpf]
#
...
echo "=== Test: filter_allow_icmp_rule (mode=$MODE) ==="
...
echo "        protocol: icmp" >> "$CONFIG_FILE"
...
if ! kill -0 $VASN_PID 2>/dev/null; then
    ERROR_MSG="vasn_tap failed to start with allow-icmp filter config"
    echo "FAIL: $ERROR_MSG"
    cat "$STATS_FILE"
    rm -f "$CONFIG_FILE" "$STATS_FILE"
    DURATION=$(($(date +%s) - START_TIME))
    JSON=$(build_result_json "test_name" "Filter (allow ICMP rule) - $MODE" "description" "ACL default_action=drop, one rule allow protocol=icmp. Expect RX>0, TX>0, Dropped<=2." "result" "$RESULT" "mode" "$MODE" "workers" "$WORKERS" "input_iface" "veth_src_host" "output_iface" "veth_dst_host" "traffic_type" "ICMP ping" "traffic_count" "$NUM_PINGS" "rx_packets" "0" "tx_packets" "0" "dropped_packets" "0" "captured_at_dst" "0" "duration_sec" "$DURATION" "error_msg" "$ERROR_MSG")
    write_result "$JSON" "filter_allow_icmp_rule_${MODE}"
    exit 1
fi
...
# Allow Dropped<=2: a few non-ICMP packets (e.g. ARP) may be dropped by the allow-icmp rule
if [ "${RX_COUNT:-0}" -gt 0 ] && [ "${TX_COUNT:-0}" -gt 0 ] && [ "${DROP_COUNT:-0}" -le 2 ]; then
    RESULT="PASS"
    echo "PASS: filter_allow_icmp_rule ($MODE)"
else
    ERROR_MSG="With allow-icmp rule expected RX>0, TX>0, Dropped<=2; got RX=${RX_COUNT:-0}, TX=${TX_COUNT:-0}, Dropped=${DROP_COUNT:-0}"
    echo "FAIL: filter_allow_icmp_rule ($MODE) - $ERROR_MSG"
fi
