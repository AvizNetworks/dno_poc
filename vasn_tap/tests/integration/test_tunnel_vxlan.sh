#!/bin/bash
#
# vasn_tap integration test - Tunnel VXLAN
# Config: filter allow all + tunnel type VXLAN to 192.168.201.1 (ns_dst peer).
# Expect: vasn_tap starts, receives pings, reports "Tunnel (VXLAN): N packets sent" with N > 0.
#
...
if [ ! -x "$VASN_TAP" ]; then
    ...
    JSON=$(build_result_json \
        "test_name"         "Tunnel VXLAN" \
        "description"      "VXLAN tunnel encap to 192.168.201.1 (ns_dst); expect Tunnel (VXLAN) packets sent > 0" \
        "result"            "$RESULT" \
        "mode"              "afpacket" \
        "workers"           "$WORKERS" \
        ...
        "error_msg"         "$ERROR_MSG")
    write_result "$JSON" "tunnel_vxlan"
    exit 1
fi
...
# Capture VXLAN (UDP 4789) packets in ns_dst to verify encapsulated frames arrived
CAPTURE_FILE=$(mktemp /tmp/vasn_tap_tunnel_XXXXXX.pcap)
ip netns exec ns_dst timeout $TIMEOUT tcpdump -i veth_dst_ns -c 50 udp port 4789 -w "$CAPTURE_FILE" 2>/dev/null &
TCPDUMP_PID=$!
sleep 0.5
...
if ! kill -0 $VASN_PID 2>/dev/null; then
    ERROR_MSG="vasn_tap failed to start with VXLAN tunnel config"
    echo "FAIL: $ERROR_MSG"
    cat "$STATS_FILE"
...
TUNNEL_PACKETS=$(grep -oP 'Tunnel \(VXLAN\): \K[0-9]+' "$STATS_FILE" | tail -1)
CAPTURED=$(tcpdump -r "$CAPTURE_FILE" 2>/dev/null | wc -l)
echo "  RX=${RX_COUNT:-0}, Tunnel (VXLAN) packets sent=${TUNNEL_PACKETS:-0}, captured at ns_dst=${CAPTURED:-0}"
...
if [ "${RX_COUNT:-0}" -gt 0 ] && [ "${TUNNEL_PACKETS:-0}" -gt 0 ]; then
    RESULT="PASS"
    echo "PASS: tunnel_vxlan"
else
    ERROR_MSG="Expected RX>0 and Tunnel (VXLAN) packets>0; got RX=${RX_COUNT:-0}, tunnel_packets=${TUNNEL_PACKETS:-0}"
    echo "FAIL: tunnel_vxlan - $ERROR_MSG"
fi
